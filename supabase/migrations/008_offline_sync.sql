-- 008_offline_sync.sql
-- ---------------------------------------------------------------------------
-- Offline-first sync layer for the three write-enrolled tables
-- (daily_vouchers, daily_voucher_lines, inventory_transactions) per the RxDB
-- Supabase Replication Plugin contract:
--   * text primary keys (uuid values preserved via ::text cast)
--   * _modified timestamptz checkpoint, trigger-updated on every change
--   * _deleted boolean soft-delete (clients must never hard-delete rows they
--     have replicated; offline clients need to learn about deletions)
--   * tables added to the supabase_realtime publication for live streaming
--
-- Also (approved Step A scope):
--   * revert_daily_voucher + submit_daily_voucher + rpc_admin_delete_
--     inventory_txn convert to soft-deletes
--   * every server-side reader of the three tables filters _deleted = false
--     (reporting + balance functions) so soft-deleted rows never leak into
--     stock/history/reports
--   * voucher-id RPC params and txn-id return columns re-typed uuid -> text
--   * user_activity_logs.resource_id widened uuid -> text (knock-on: entity
--     ids referenced there are now text for vouchers)
-- ---------------------------------------------------------------------------

-- ============ 1. _modified trigger helper ============
CREATE OR REPLACE FUNCTION public.touch_offline_modified()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $function$
BEGIN
...
-- The daily_voucher_lines RLS policies reference daily_vouchers.id and
-- daily_voucher_lines.voucher_id via subqueries; Postgres forbids re-typing a
-- column used in a policy definition. Drop them first, re-create after the casts.
DROP POLICY IF EXISTS "daily_voucher_lines_operator_manage" ON public.daily_voucher_lines;
DROP POLICY IF EXISTS "merged_daily_voucher_lines_select" ON public.daily_voucher_lines;

-- daily_vouchers
ALTER TABLE public.daily_vouchers
...
-- daily_voucher_lines: drop FK, cast both sides, re-add
ALTER TABLE public.daily_voucher_lines
  DROP CONSTRAINT IF EXISTS daily_voucher_lines_voucher_id_fkey;
ALTER TABLE public.daily_vouchers
  ALTER COLUMN id TYPE text USING id::text;
ALTER TABLE public.daily_voucher_lines
  ALTER COLUMN id TYPE text USING id::text,
  ALTER COLUMN voucher_id TYPE text USING voucher_id::text;
ALTER TABLE public.daily_voucher_lines
  ADD CONSTRAINT daily_voucher_lines_voucher_id_fkey
  FOREIGN KEY (voucher_id) REFERENCES public.daily_vouchers(id) ON DELETE CASCADE;
ALTER TABLE public.daily_voucher_lines
  ADD COLUMN IF NOT EXISTS _modified timestamptz NOT NULL DEFAULT now(),
  ADD COLUMN IF NOT EXISTS _deleted boolean NOT NULL DEFAULT false;

-- inventory_transactions: id + loose source_id reference (voucher ids)
...
-- user_activity_logs.resource_id: widening (voucher ids are now text)
ALTER TABLE public.user_activity_logs
  ALTER COLUMN resource_id TYPE text USING resource_id::text;

-- Re-create the two dropped RLS policies (same definitions as before).
...
-- ============ 3. _modified triggers ============
CREATE TRIGGER trg_daily_vouchers_offline_modified
  BEFORE UPDATE ON public.daily_vouchers FOR EACH ROW
  EXECUTE FUNCTION public.touch_offline_modified();
CREATE TRIGGER trg_daily_voucher_lines_offline_modified
  BEFORE UPDATE ON public.daily_voucher_lines FOR EACH ROW
  EXECUTE FUNCTION public.touch_offline_modified();
CREATE TRIGGER trg_inventory_transactions_offline_modified
  BEFORE UPDATE ON public.inventory_transactions FOR EACH ROW
  EXECUTE FUNCTION public.touch_offline_modified();

-- ============ 4. filter-friendly indexes ============
CREATE INDEX IF NOT EXISTS idx_daily_vouchers_offline_modified
  ON public.daily_vouchers (_modified);
CREATE INDEX IF NOT EXISTS idx_daily_voucher_lines_offline_modified
  ON public.daily_voucher_lines (_modified);
CREATE INDEX IF NOT EXISTS idx_inventory_transactions_offline_modified
  ON public.inventory_transactions (_modified);
-- 24h window filter or=(voucher_date.gte.<today-1>,status.eq.draft)
CREATE INDEX IF NOT EXISTS idx_daily_vouchers_status_draft
  ON public.daily_vouchers (status) WHERE _deleted = false;

-- ============ 5. realtime publication (idempotent) ============
DO $do$
BEGIN
  ALTER PUBLICATION supabase_realtime ADD TABLE public.daily_vouchers;
EXCEPTION WHEN duplicate_object THEN NULL;
END;
$do$;
DO $do$
BEGIN
  ALTER PUBLICATION supabase_realtime ADD TABLE public.daily_voucher_lines;
EXCEPTION WHEN duplicate_object THEN NULL;
END;
$do$;
DO $do$
...
-- ============ 6. drop superseded uuid-signature overloads ============
DROP FUNCTION IF EXISTS public.submit_daily_voucher(uuid, uuid, date, jsonb, boolean);
DROP FUNCTION IF EXISTS public.save_daily_sheet(uuid, jsonb);
DROP FUNCTION IF EXISTS public.rpc_upsert_voucher_line(uuid, uuid, text, numeric, text, numeric, numeric, text, jsonb, uuid);
DROP FUNCTION IF EXISTS public.revert_daily_voucher(uuid);
DROP FUNCTION IF EXISTS public.rpc_get_or_create_draft_voucher(uuid, date, text);
DROP FUNCTION IF EXISTS public.rpc_admin_delete_inventory_txn(uuid);
DROP FUNCTION IF EXISTS public.rpc_admin_update_inventory_txn(uuid, numeric, numeric, date, text, text, numeric);
-- return-type changes (txn id uuid -> text): CREATE OR REPLACE cannot change RETURNS, so drop first
...
CREATE OR REPLACE FUNCTION public.reporting_purchases_v3(p_date_from date DEFAULT NULL::date, p_date_to date DEFAULT NULL::date, p_farm_id uuid DEFAULT NULL::uuid, p_supplier_id uuid DEFAULT NULL::uuid, p_item_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(txn_id text, txn_date date, supplier_id uuid, supplier_name text, item_id uuid, item_name text, item_unit text, qty numeric, unit_price numeric, total_amount numeric, reference_no text)
 LANGUAGE sql
 STABLE
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.reporting_sales_transfers_v3(p_date_from date DEFAULT NULL::date, p_date_to date DEFAULT NULL::date, p_farm_id uuid DEFAULT NULL::uuid, p_item_id uuid DEFAULT NULL::uuid, p_txn_type text DEFAULT NULL::text)
 RETURNS TABLE(txn_id text, txn_date date, txn_type text, source_farm text, dest_farm text, customer_name text, item_id uuid, item_name text, item_unit text, qty numeric, unit_price numeric, amount numeric, reference_no text)
 LANGUAGE sql
 STABLE
AS $function$
  SELECT t.id                                   AS txn_id,
...
CREATE OR REPLACE FUNCTION public.rpc_upsert_voucher_line(p_voucher_id text, p_item_id uuid, p_formula_no text, p_mixer_count numeric, p_hall_numbers text, p_consumed numeric, p_waste numeric, p_notes text, p_hall_consumed jsonb, p_formula_id uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.rpc_admin_delete_inventory_txn(p_txn_id text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  UPDATE public.inventory_transactions SET _deleted = true WHERE id = p_txn_id;
END;
$function$;

CREATE OR REPLACE FUNCTION public.rpc_admin_update_inventory_txn(p_txn_id text, p_qty_in numeric, p_qty_out numeric, p_txn_date date, p_notes text, p_reference text, p_unit_price numeric)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.revert_daily_voucher(p_voucher_id text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- ============ 8. remaining server-side readers (added post-generation review) ============
-- These two functions read inventory_transactions but were missed by the automated
-- transform; without a _deleted = false filter they would count soft-deleted rows
-- (blocking initial-stock re-creation and supplier deletion after a revert).
